...
import { closeEditPromptModal } from "@/redux/features/modal/modalSlice";
...
const EditPromptModal = () => {
    ...
    const isOpen = useSelector(
        (state: RootState) => state.modal.isEditPromptOpen
    );

    const promptId = useSelector(
        (state: RootState) => state.modal.selectedPromptId
    );
    ...
    const prompt = useSelector((state: RootState) =>
        state.prompt.prompts.find((item) => item.id === promptId)
    );

    const { loading, submit } = useUpdatePrompt();
    ...
    // Populate form whenever prompt changes
    useEffect(() => {
        if (!prompt) return;
